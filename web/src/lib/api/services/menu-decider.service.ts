...
export const menuDeciderService = {
  async getDishRecommendations(userId: string, venueId: string) {
    ...
    for (const note of notes) {
      ...
      const dishName = ext?.dishName as string | undefined;
      ...
      if (!dishMap.has(key)) {
        dishMap.set(key, {
          dishName,
          dishCategory: (ext.dishCategory as string) || 'OTHER',
          notes: [],
        });
      }
      ...
      dishMap.get(key)!.notes.push({
        ...
        wouldOrderAgain: (ext.wouldOrderAgain as boolean) ?? false,
        tier,
      });
    }
